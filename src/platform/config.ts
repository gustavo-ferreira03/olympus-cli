...
/** Guard a locally cached JSON file before parsing it. */
function assertLocalStateWithinLimit(path: string, label: string): void {
  assertFileWithinLimit(path, MAX_LOCAL_STATE_BYTES, label);
}
...
function isValidCliConfig(value: unknown): value is CliConfig {
  ...
  return (
    typeof candidate.convexUrl === "string"
...
export function getBaseUrl(): string {
  if (process.env.OLYMPUS_URL) return process.env.OLYMPUS_URL;
...
async function getConfig(signal?: AbortSignal): Promise<CliConfig> {
  if (existsSync(CONFIG_PATH)) {
    try {
      assertLocalStateWithinLimit(CONFIG_PATH, "Config cache");
      const cached = JSON.parse(readFileSync(CONFIG_PATH, "utf-8"));
      if (isValidCliConfig(cached)) {
        const age = Date.now() - new Date(cached.fetchedAt).getTime();
        if (age < CONFIG_TTL_MS) return cached;
      }
    } catch {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (
        ...
        throw new Error(
          `CLI config from ${baseUrl}/api/cli/config is missing convexUrl.\n`
  ...
  throw new Error(
    `Could not fetch CLI config from ${baseUrl}/api/cli/config\n`
...
export async function getConvexUrl(): Promise<string> {
  if (process.env.OLYMPUS_CONVEX_URL) return process.env.OLYMPUS_CONVEX_URL;
...
async function getLatestPublishedVersion(signal?: AbortSignal): Promise<string | null> {
  if (!UPDATE_PACKAGE_NAME) return null;
  if (existsSync(VERSION_CACHE_PATH)) {
    try {
      assertLocalStateWithinLimit(VERSION_CACHE_PATH, "Version cache");
      const cached = JSON.parse(readFileSync(VERSION_CACHE_PATH, "utf-8"));
      if (isValidVersionCache(cached)) {
        const age = Date.now() - new Date(cached.fetchedAt).getTime();
        if (age < VERSION_CACHE_TTL_MS) return cached.latestVersion;
      }
    } catch {
      // Stale or corrupt — refetch
    }
...
export async function checkVersion(signal?: AbortSignal) {
  if (process.env.OLYMPUS_NO_UPDATE_CHECK === "1") return;
