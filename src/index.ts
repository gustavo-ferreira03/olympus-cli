...
import { CliError, describeError, sanitizeDiagnostic } from "./shared/errors.ts";
import { installDiagnosticOutput } from "./terminal/diagnostics.ts";
import { createSchemaCommand } from "./commands/schema.ts";
...
const update = defineCommand({
  ...
  run: async ({ args }) => {
    if (!UPDATE_PACKAGE_NAME) {
      throw new CliError("Self-update is disabled for this private fork.", {
        kind: "config",
        code: "update.disabled",
        retryable: false,
        hint: "Update from your repository and run pnpm install && pnpm build, or configure OLYMPUS_UPDATE_PACKAGE for your published fork.",
      });
    }
    ...
    try {
      ...
      throw new CliError("Update failed.", {
        ...
        code: "update.failed",
        retryable: null,
        hint: `Inspect the npm failure before retrying npm install -g ${spec}.`,
      });
    }
  },
});
