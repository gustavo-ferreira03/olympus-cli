...
import {
  assertRemoteReevaluationAttempts,
  inspectReevaluationHistory,
  assertRunCount,
  assertPaidEndpoint,
  assertRunCapacity,
  assertRunPreset,
...
import { api, getClient, requireProblemVersion } from "../platform/convex.ts";
import { printJson, printKeyValue, printTable, statusBadge, truncate } from "../terminal/format.ts";
import { omitEmpty, paginate, parsePositiveInteger } from "../terminal/output.ts";
import {
  type AgentRun,
  formatAgentType,
  formatRunLabel,
...
import { type Client, type CriteriaReport, type ProblemVersion } from "../shared/types.ts";
import { resolveWaitWindow, waitWindow } from "../shared/wait.ts";
import { printArtifact } from "../core/artifacts.ts";

/** Rollouts poll less often and for longer than checks do. */
const RUN_WAIT_DEFAULTS = { intervalSeconds: 10, timeoutMinutes: 120 } as const;
...
function formatStatusSummary(statusCounts: Map<string, number>): string {
  return [...statusCounts.entries()]
    .toSorted(([a], [b]) => a.localeCompare(b))
    .map(([status, count]) => `${count} ${status}`)
    .join(", ");
}
function summarizeConfigMix(configs: RunConfig[]): string[] {
...
function parseAgentTypeOrExit(value: string | undefined, flag: string): string {
  const parsed = parseAgentTypeInput(value);
  if (!parsed) {
    throw new CliError(`Invalid --${flag} value: ${value ?? "(missing)"}`, {
      kind: "usage",
      code: "input.invalid_agent",
      retryable: false,
      hint: `Use one of: ${AGENT_INPUT_HELP}`,
    });
  }
  return parsed;
}
function printCriteria(criteria: CriteriaReport): void {
...
function printRunGroups(runs: AgentRun[]): void {
  if (runs.length === 0) {
    console.log("\n  No rollouts yet.");
    return;
  }
  for (const group of groupRunsByBatch(runs)) {
    const statusSummary = formatStatusSummary(
      summarizeStatuses(group.runs.map((run) => run.status)),
    );
    const meta = [
...
function findAgentRunKey(config: RunConfig): string | undefined {
  return Object.entries(AGENT_RUN_CONFIGS).find(
    ([, candidate]) =>
      candidate.taskAgentType === config.taskAgentType
      && candidate.evalAgentType === config.evalAgentType,
  )?.[0];
}

function buildConfigs(args: TriggerBatchOptions): RunConfig[] {
...
async function assertRolloutsAllowed(
  client: Awaited<ReturnType<typeof getClient>>,
  problemId: string,
) {
  const readiness: any = await client.query(api.submissionReadiness.getSubmissionReadiness, {
    problemId,
  });
  ...
  const blockers = (readiness?.criteria ?? [])
    .filter(
      (criterion: any) =>
        required.has(criterion.id) && criterion.status !== "pass" && criterion.status !== "warn",
    )
...
async function triggerBatch(args: TriggerBatchOptions) {
  ...
  if (
    args.preset
    && (args.solver !== undefined || args.evaluator !== undefined || args.count !== undefined)
  ) {
  ...
  const configs: RunConfig[] =
    args.preset === "quick" || args.preset === "full"
...
const presets = defineCommand({
  ...
  run: async ({ args }) => {
    ...
    console.log(`\n  Custom runs:`);
    console.log(
      `    \x1b[90molympus runs run ${args.id} --solver nova --evaluator orion --count 3\x1b[0m`,
    );
  ...
  run: async ({ args }) => {
    ...
    if (args.only) {
      normalizedRuns = normalizedRuns.filter((run) => {
        ...
        if (args.only === "passing")
          return run.status === "completed" && verdict.startsWith("PASS");
        ...
        if (args.batch) {
          ...
          const batches = groupRunsByBatch(normalizedRuns).map((group) => {
            ...
            const verdicts = Object.fromEntries(
              summarizeStatuses(group.runs.map((run) => String(run.output?.verdict ?? "unknown"))),
            );
            return omitEmpty({
    ...
    console.log("\n  Commands:");
    console.log(
      `    olympus runs presets ${args.id}                 Show quick/full rollout mixes`,
    );
    console.log(`    olympus runs run ${args.id} --solver nova --evaluator orion --count 3`);
    console.log(
      `    olympus runs run ${args.id} --preset full      Trigger the production full preset`,
    );
...
const show = defineCommand({
  ...
  run: async ({ args }) => {
    ...
    const runs = normalizeAgentRuns(
      await client.query(api.runAgentRuns.getAgentRuns, {
        versionId: version._id,
      }),
    );
    ...
    printKeyValue([
      ...
      ["Trajectory", output.trajectoryLength == null ? null : String(output.trajectoryLength)],
      ["Time", output.executionTimeSeconds == null ? null : `${output.executionTimeSeconds}s`],
...
const run = defineCommand({
  ...
  run: async ({ args }) => {
    const waitWindow = resolveWaitWindow(args, RUN_WAIT_DEFAULTS);
...
async function queryRunsWithRetry(client: Client, versionId: string): Promise<AgentRun[]> {
  let lastError: unknown;
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      return normalizeAgentRuns(await client.query(api.runAgentRuns.getAgentRuns, { versionId }));
    } catch (error) {
      lastError = error;
      if (attempt < 3) {
...
/** Options accepted by {@link triggerBatch}. */
type TriggerBatchOptions = {
  problemId: string;
  solver?: string;
  evaluator?: string;
  count?: string | number;
  ...
  hinted?: boolean;
  batchName?: string;
  useGeneralTokens?: boolean;
};

/** Options accepted by {@link waitForRuns}. */
type WaitForRunsOptions = {
  client: Client;
  ...
  while (true) {
    ...
    if (targetIds === null) {
      if (jobId) {
        ...
        if (!match)
          throw new Error(`Run job ${jobId} was not found on current v${version.version}`);
        targetIds = [match.id];
      } else if (runSelector) {
        ...
        if (!resolved.run)
          throw new Error(`Run "${runSelector}" was not found on current v${version.version}`);
    ...
    if (active.length === 0) {
      ...
      if (json) printJson(result);
        ...
        for (const run of summaries) {
          ...
          console.log(
            `${statusBadge(run.status)}  ${run.label}${detail ? ` — ${truncate(String(detail), 100)}` : ""}`,
          );
        }
      }
    ...
    if (!json) {
      const progress = active
        ...
        .join(" ");
      process.stderr.write(
        `\r  waiting runs=${active.length} elapsed=${elapsedSeconds}s ${progress}`,
      );
    }
...
const wait = defineCommand({
  ...
  run: async ({ args }) => {
    ...
    const window = waitWindow(args, RUN_WAIT_DEFAULTS);
    const client = await getClient();
    const { version } = await requireProblemVersion(client, args.id);
    await waitForRuns({
      client,
...
const reEvaluateView = defineCommand({
  ...
  args: {
    history: {
      type: "boolean",
      description: "Fingerprint remote solution patches and count re-evaluation batches",
    },
    id: { type: "positional", description: "Challenge ID", required: true },
    json: { type: "boolean", description: "Output as JSON" },
  },
...
const reEvaluateRun = defineCommand({
  ...
  run: async ({ args }) => {
    ...
    assertPaidEndpoint("reEvalRuns:triggerReEvalRuns", {
      useGeneralTokens: args["use-general-tokens"],
    });
...
const artifact = defineCommand({
  ...
  run: async ({ args }) => {
    ...
    await printArtifact({
      ...
      ownerDescription: `rollout "${args.run}"`,
      jsonIdentity: { run: run.id },
      fullCommand: `olympus runs artifact ${args.id} ${run.id} --key=${args.key} --full${args.json ? " --json" : ""}`,
      args,
    });
  },
});
