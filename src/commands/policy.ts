...
import { mkdirSync, writeFileSync, existsSync, readFileSync, renameSync, unlinkSync, lstatSync, openSync, closeSync, constants } from "node:fs";
import { randomUUID } from "node:crypto";
import { spawnSync } from "node:child_process";
import { parseDocument } from "yaml";
import { dirname, resolve } from "node:path";
import { defaultPolicyYaml, loadPolicy, parsePolicy, policyPath, policySchema } from "../policy.ts";
...
const init = defineCommand({
  ...
  args: {
    ...
    "schema-only": { type: "boolean", description: "Refresh editor schema without changing policy values" },
  },
  run: ({ args }) => {
    const path = policyPath();
    mkdirSync(dirname(path), { recursive: true, mode: 0o700 });
    if (existsSync(path) && !args["schema-only"]) throw new Error(`Guardrails file already exists: ${path}`);
...
const edit = defineCommand({
  ...
  args: {
    key: { type: "positional", description: "Optional dotted key, such as tokens.challenge_budget", required: false },
    value: { type: "positional", description: "New YAML value, such as 100, false, null or '[verifyTests]'", required: false },
    json: { type: "boolean", description: "Output JSON; requires key and value (no interactive editor)" },
  },
  run: ({ args }) => {
    const direct = args.key !== undefined;
    if (direct !== (args.value !== undefined)) throw new Error("Provide both a policy key and a YAML value, or neither to open the editor");
    if (direct && !args.value!.trim()) throw new Error("Provide an explicit YAML value; use null to disable an optional rule");
    if (!direct && args.json) throw new Error("JSON mode requires a policy key and a YAML value");
    const editor = process.env.VISUAL?.trim() || process.env.EDITOR?.trim() || (process.stdin.isTTY ? "vi" : undefined);
    if (!direct && !editor) throw new Error("Set VISUAL or EDITOR, or use policy edit <key> <value>");
    const path = policyPath();
    const directory = dirname(path);
    mkdirSync(directory, { recursive: true, mode: 0o700 });
    const lockFd = openSync(resolve(directory, ".policy-edit.flock"), constants.O_CREAT | constants.O_RDWR | constants.O_NOFOLLOW, 0o600);
    const draft = resolve(directory, `.policy-edit-${randomUUID()}.yml`);
    try {
      const acquired = spawnSync("flock", ["-n", "3"], { stdio: ["ignore", "ignore", "pipe", lockFd] });
      if (acquired.error || (acquired.status !== 0 && acquired.status !== 1)) throw new Error("Cannot acquire the edit lock; policy edit requires working flock (util-linux)");
      ...
      if (direct) {
        ...
        if (doc.errors.length || doc.warnings.length || value.errors.length || value.warnings.length || value.contents === null) throw new Error("Invalid or empty YAML value; policy unchanged");
      ...
      if (!direct) {
        writeFileSync(resolve(directory, "policy.schema.json"), JSON.stringify(policySchema(), null, 2) + "\n", { mode: 0o600 });
        const result = spawnSync("/bin/sh", ["-c", `${editor} "$1"`, "policy-editor", draft], { stdio: "inherit" });
        if (result.error || result.status !== 0) throw new Error("Editor failed or was interrupted; policy unchanged");
        text = readFileSync(draft, "utf8");
      }
      const policy = parsePolicy(text);
      const current = existsSync(path) ? readFileSync(path, "utf8") : undefined;
      if (current !== original) throw new Error("Policy changed while editing; refusing to overwrite concurrent changes");
      writeFileSync(resolve(directory, "policy.schema.json"), JSON.stringify(policySchema(), null, 2) + "\n", { mode: 0o600 });
      ...
      try { if (existsSync(draft)) unlinkSync(draft); }
