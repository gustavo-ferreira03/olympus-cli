...
import { getBaseUrl } from "../platform/config.ts";
...
const fail = (detail: string): never => {
  throw new Error(`Official pricing unavailable: ${detail}`);
};
const key = (n: Node): string | undefined =>
  n?.type === "Identifier"
    ? n.name
    : n?.type === "Literal" && typeof n.value === "string"
      ? n.value
      : undefined;
const props = (n: Node): Map<string, Node> =>
  new Map(
    n?.type === "ObjectExpression"
      ? n.properties
          .filter((p: Node) => p.type === "Property" && !p.computed && p.kind === "init")
          .map((p: Node) => [key(p.key), p.value])
      : [],
  );
const member = (n: Node, name: string) =>
  n?.type === "MemberExpression" && !n.computed && key(n.property) === name;
 ...
 */
function parseOfficialPrices(source: string): OfficialPrices {
  ...
  const bindPattern = (n: Node, scope: Scope) => {
    ...
    if (n.type === "Identifier") scope.bindings.set(n.name, null);
      ...
      n.properties.forEach((p: Node) => bindPattern(p.value ?? p.argument, scope));
  ...
  const walk = (n: Node, scope: Scope, parent?: Node) => {
    ...
    if (
      /^(FunctionDeclaration|FunctionExpression|ArrowFunctionExpression|BlockStatement|CatchClause|ForStatement|ForInStatement|ForOfStatement|SwitchStatement)$/.test(
        n.type,
      )
    ) {
      scope = {
        parent: scope,
        functionScope: /^(FunctionDeclaration|FunctionExpression|ArrowFunctionExpression)$/.test(
          n.type,
        ),
        bindings: new Map(),
      };
    ...
    if (n.type === "VariableDeclaration")
      for (const d of n.declarations) {
        ...
        if (n.kind === "var")
          while (declarationScope.parent && !declarationScope.functionScope)
            declarationScope = declarationScope.parent;
        bindPattern(d.id, declarationScope);
        if (d.id.type === "Identifier" && n.kind === "const")
          declarationScope.bindings.set(d.id.name, d.init);
      }
    for (const v of Object.values(n)) {
      if (Array.isArray(v))
        v.forEach((c) => {
          if (c?.type) walk(c, scope, n);
        });
  ...
  const binding = (n: Node): Node | null | undefined => {
    for (let s = scopes.get(n); s; s = s.parent)
      if (s.bindings.has(n.name)) return s.bindings.get(n.name);
  };
  ...
  const strictProps = (n: Node) => {
    const p = props(n);
    if (n?.type !== "ObjectExpression" || p.size !== n.properties.length || mutated.has(n))
      fail("unsupported, duplicate or mutated constant object");
    return p;
  };
  const resolve = (n: Node, seen = new Set<Node>()): Node => {
    ...
    if (n.type === "MemberExpression" && !n.computed)
      return resolve(strictProps(resolve(n.object, seen)).get(key(n.property)!), seen);
    return n;
  };
  const value = (n: Node): number => {
    const r = resolve(n);
    if (r.type !== "Literal" || !amount(r.value))
      return fail("price is not a finite non-negative static number");
    return r.value;
  };
  const one = <T>(items: T[], label: string): T =>
    items.length === 1 ? items[0] : fail(`missing or ambiguous ${label} (${items.length} matches)`);
  ...
  const base = one(
    objects.filter((n) =>
      ...
      ].every((k) => props(n).has(k)),
    ),
    "base check catalog",
  );
  strictProps(base);
  const checks = Object.fromEntries([...props(base)].map(([k, n]) => [k, value(n)]));
  const containsLabel = (n: Node, label: string): boolean =>
    n?.type === "Literal"
      ? n.value === label
      : n?.type === "ConditionalExpression"
        && (containsLabel(n.consequent, label) || containsLabel(n.alternate, label));
  const dialog = (label: string) => {
    const result = one(
      objects.filter((n) => props(n).has("cost") && containsLabel(props(n).get("label"), label)),
      label,
    );
    strictProps(result);
    return result;
  ...
  const precheck = one(
    objects.filter((n) => {
      ...
      try {
        return resolve(k).value === "bundled_prechecks";
      } catch {
        return false;
      }
    }),
  ...
  const pcost = preDialog.get("cost"),
    plabel = preDialog.get("label");
  if (
    pcost?.type !== "ConditionalExpression"
    || plabel?.type !== "ConditionalExpression"
    || source.slice(pcost.test.start, pcost.test.end)
      !== source.slice(plabel.test.start, plabel.test.end)
  )
    fail("unsupported precheck spend formula");
  if (
    value(plabel.consequent.value === "Run Prechecks" ? pcost.consequent : pcost.alternate)
    !== bundledPrechecks
  )
    fail("conflicting precheck prices");
  const buildMembers = nodes.filter(
    (n) => member(n, "buildVersionImage") && member(n.object, "dockerImage"),
  );
  ...
  for (const m of buildMembers) {
    ...
    if (
      ...
      && n.params?.some(
        (p: Node) =>
          p.type === "ObjectPattern" && p.properties.some((q: Node) => key(q.key) === "cost"),
      )
    )
      buildFunctions.add(n);
  }
  ...
  const calls = nodes.filter(
    (n) =>
      ...
      && binding(n.arguments[0]) === buildFunction
      && props(n.arguments[1]).has("cost"),
  );
  const build = value(strictProps(one(calls, "build cost prop").arguments[1]).get("cost"));
  const diamond: Record<string, number> = {};
  for (const field of [
    "diamondChecksCostNewSession",
    "diamondChecksCostAppendJob",
    "diamondChecksCostCodeValidation",
    "diamondChecksCostFullEnvQa",
  ]) {
    const matches = nodes.filter(
      (n) =>
        n.type === "LogicalExpression"
        && n.operator === "??"
        && member(n.left?.type === "ChainExpression" ? n.left.expression : n.left, field),
    );
...
async function fetchOfficialPrices() {
  ...
  for (const m of html.matchAll(/<script\b([^>]*)>([\s\S]*?)<\/script\s*>/gi)) {
    ...
    if (
      ...
      return fail("unsupported inline module import");
  ...
  if (
    assetUrl.origin !== pageUrl.origin
    ...
    return fail("frontend asset is not same-origin HTTP(S)");
  const source = await readText(assetUrl, 16 * 1024 * 1024);
  return {
    ...parseOfficialPrices(source),
    source: {
...
const known = (tokens: unknown, source: string, baseTokens?: number): Price =>
  amount(tokens)
    ? { tokens, source, ...(baseTokens === undefined ? {} : { baseTokens }) }
    : { tokens: null, source, reason: "No valid prospective price returned" };
export type PricingReader = { query: (ref: any, args: any) => Promise<any> };
export async function resolveVersionOffer(
  client: PricingReader,
  kind: "fp" | "reevaluation",
  versionId?: string,
) {
  const source =
    kind === "fp"
      ? "fpReview:getFpCheckForVersion.tokenCost"
      : "reEvalRuns:getReEvalOffer.tokenCost";
  let offer: any;
  if (versionId) {
    try {
      offer = await client.query(
        kind === "fp" ? anyApi.fpReview.getFpCheckForVersion : anyApi.reEvalRuns.getReEvalOffer,
        { versionId },
      );
    } catch {
      /* Read failure is unknown, never a free offer or a historical fallback. */
    }
  }
  return {
    ...
    ...(kind === "fp"
      ? { canRun: offer?.canRun ?? null }
      : { eligible: offer?.eligible ?? null, runCount: offer?.runCount ?? null }),
  };
}
function runPrices(config: any): Record<string, Price> {
  return Object.fromEntries(
    ["claude_code", "codex_cli", "gemini_cli", "taiga"].map((k) => [
      k,
      known(
        object(config?.agentRunPricing) && Object.hasOwn(config.agentRunPricing, k)
          ? config.agentRunPricing[k]
          : undefined,
        `questConfig.agentRunPricing.${k}`,
      ),
    ]),
  );
}
...
export async function resolveCostCatalog(client: PricingReader, versionId?: string) {
  ...
  for (const [k, base] of Object.entries(ui.checks)) {
    ...
    checks[k] = known(
      accepted ? override : base,
      accepted ? `questConfig.checkTokenCostOverrides.${k}` : "official-ui:check-base",
      base,
    );
  }
  const runs = runPrices(config);
  const diamond = Object.fromEntries(
    Object.entries(ui.diamond).map(([field, base]) => [
      field,
      known(
        config[field] ?? base,
        config[field] == null ? "official-ui:diamond-default" : `questConfig.${field}`,
        base,
      ),
    ]),
  );
  const unknown = {
    tokens: null,
    source: "unknown",
    reason: "No official prospective tariff established; not assumed free",
  } satisfies Price;
  return {
    source: ui.source,
    unit: "contributor tokens",
    prospective: true,
    checks,
    runs,
    actions: {
    ...
    formulas: {
      runs: "sum(solver price per requested run)",
      finalQa: "finalQaPerItem * selected or default eligible item count",
      finalQaRetry: "finalQaPerItem * failed/cancelled/stale-success item count",
      reevaluation: "total version offer, not a per-run tariff",
      bundledPrechecks: "one charge per bundled dispatch, not per stage",
    },
    unknown: Object.fromEntries(
      [
        ...
        "fpReview:contestFpCheck",
        "verifierIncompleteness:submitVerifierIncompletenessDecision",
        "dockerImage:cancelBuildJob",
        "runAgentRuns:cancelRun",
        "runAgentRuns:scratchRun",
      ].map((k) => [k, unknown]),
    ),
    notes: [
      "Live frontend tariffs and backend offers are prospective, not a server reservation or invoice.",
      "No historical ledger inference, persistent price cache, or stale fallback.",
      "Administrative and contest prices are unknown, not zero.",
    ],
  };
}
