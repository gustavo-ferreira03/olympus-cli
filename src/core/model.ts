 ...
 */
export const CONTESTABLE_CHECK_KEYS = ["descriptionQuality", "solutionQuality", "testQuality"];
...
export const AGENT_TYPE_CHOICES = Object.keys(AGENT_TYPE_ALIASES).filter(
...
export function parseAgentTypeInput(value: string | undefined): string | undefined {
  ...
  if (!Object.prototype.hasOwnProperty.call(AGENT_TYPE_ALIASES, key)) return undefined;
  const alias = AGENT_TYPE_ALIASES[key];
  return typeof alias === "string" ? alias : undefined;
}
function canonicalizeRunLabel(label: string | undefined): string {
  if (!label) return "";
...
export function getDynamicCheckEntries(dynamicChecks: unknown): DynamicCheckEntry[] {
  ...
  const payload: Record<string, unknown> =
    dynamicChecks && typeof dynamicChecks === "object" && !Array.isArray(dynamicChecks)
      ? (dynamicChecks as Record<string, unknown>)
      : {};
  const contestedFor = (key: string): boolean | undefined => {
    if (!CONTESTABLE_CHECK_KEYS.includes(toPublicCheckKey(key))) return undefined;
  ...
  return (
    Object.entries(record)
      // The spread builds a new entry from a caller-owned record; mutating
      // `value` in place would corrupt the input.
      ...
      .toSorted((a, b) => {
        const orderDelta =
          orderIndex(DYNAMIC_CHECK_ORDER, a.key) - orderIndex(DYNAMIC_CHECK_ORDER, b.key);
...
export function formatRunLabel(
  run: Pick<AgentRun, "label" | "taskAgentType" | "evalAgentType" | "hinted">,
): string {
...
export function groupRunsByBatch(runs: AgentRun[]): AgentRunGroup[] {
  ...
  return [...groups.entries()]
    .map(([key, groupRuns]) => {
      const sortedRuns = [...groupRuns].toSorted((a, b) => {
        if (!!a.stale !== !!b.stale) return a.stale ? 1 : -1;
    ...
    .toSorted((a, b) => {
      if (a.staleOnly !== b.staleOnly) return a.staleOnly ? 1 : -1;
...
export function resolveRunSelector(
  runs: AgentRun[],
  selector: string,
): { run?: AgentRun; matches?: AgentRun[] } {
  const normalizedSelector = selector.trim().toLowerCase();
  if (!normalizedSelector) return { run: undefined };
  const exactId = runs.find((run) => String(run.id).toLowerCase() === normalizedSelector);
  if (exactId) return { run: exactId };
  const exactLabelMatches = runs.filter(
    (run) => formatRunLabel(run).toLowerCase() === normalizedSelector,
  );
  if (exactLabelMatches.length === 1) return { run: exactLabelMatches[0] };
  if (exactLabelMatches.length > 1) return { matches: exactLabelMatches };
  const prefixMatches = runs.filter((run) =>
    String(run.id).toLowerCase().startsWith(normalizedSelector),
  );
  if (prefixMatches.length === 1) return { run: prefixMatches[0] };
  if (prefixMatches.length > 1) return { matches: prefixMatches };
